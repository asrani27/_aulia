        ...
        <div class="card">
            ...
            <form method="POST" action="/superadmin/verifikasi/edit/{{$data->id}}">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label for="exampleInputEmail1">Kode Dokumen</label>
                        <select class="form-control" name="dokumen_id" required>

                            @foreach (dokumen() as $item)
                            <option value="{{$item->id}}" {{$data->dokumen_id == $item->id ?
                                'selected':''}}>DOK-{{$item->id}},
                                {{$item->nama}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control" name="status" required>
                            ...
                            <option value="TIDAK VALID" {{$data->status == 'TIDAK VALID' ? 'selected':''}}>TIDAK VALID
                            </option>
                        </select>
                    </div>
                    <div class="form-group">

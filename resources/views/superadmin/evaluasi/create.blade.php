...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <form method="POST" action="/superadmin/evaluasi/add">
                @csrf
                <div class="card-body">
                    <div class="form-group">
                        <label for="exampleInputEmail1">Tanggal Evaluasi</label>
                        <input type="date" name="tanggal" class="form-control"
                            value="{{\Carbon\Carbon::now()->format('Y-m-d')}}">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">Kode Verifikasi</label>
                        <select class="form-control" name="verifikasi_id" required>

                            @foreach (verifikasi() as $item)
                            <option value="{{$item->id}}">EVAL-{{$item->id}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">Hasil Evaluasi / Keterangan</label>
                ...
                <div class="card-footer">
                    ...
                    <a href="/superadmin/evaluasi" class="btn btn-danger">Kembali</a>
                </div>
            </form>
            <!-- /.card-body -->
        </div>
